...
Here a 2D array is formed by reshaping the 1D array `a` into a $3 \times 2$ array where the elements are reshaped using <a href="https://en.wikipedia.org/wiki/Row-_and_column-major_order" target="_blank">**column-major order**</a> so the first three elements `a` form the first column of the 2D array, the next three elements form the second column of the 2D array and so on.

The other way is to use **row-major order** where the row of the reshaped array are formed by taking successive elements from the original array. To do this in MATLAB we simply transpose the array we are reshaping and swap the `rows` and `columns` inputs around.
