...
Rerun your program and the following should be added to the command window.
...
Rerun your program and you should see the command window output has changed to the following.
...
Here we have used the `super()` function to access the `print()` function from the parent class `Method` and then added a line to print the `position` attribute from the child class `Student`. Rerun your program and you should see that the command window output hasn't changed.
...
Run your program and you should see the command window output changed to the following.
...
```{exercise}
:label: matlab-oop-exercise-5

Declare the following classes that inherits from the `Vehicle` class from {ref}`matlab-oop-exercise-1`: 
...
```{exercise}
:label: matlab-oop-exercise-6

Declare the following classes that inherit from the `Shape` class from {ref}`matlab-oop-exercise-2`:
