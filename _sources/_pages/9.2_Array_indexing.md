...
`start` is the index of the first element in the range, `stop` is the index of the last element in the range and `step` is the difference between the indices. If any these are unspecified, MATLAB uses default values of `start = 1`, `stop = array length`, `step = 1`.
...
To print the middle 3 elements of `b` we use `start = 4` (since array indexing starts at 1, this is the 4th element of `b`) and `stop = 6`. As we want a step of 1, we don't need to specify a value for `step`. Enter the following code to your program. 
